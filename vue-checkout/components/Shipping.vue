	...
	<v-expansion-panel>
		...
		<v-expansion-panel-content v-if="cartItemTotal">
			...
			>
				...
				<span v-else>Shipping Address</span>
				<a v-if="store_config.checkoutSettings.hasMultiShippingEnabled" @click="toggleMultiShipping">
					<span v-if="multiShipOrder">Ship to a single address</span>
					<span v-else>Ship to multiple addresses</span>
				</a>
			</div>
...
export default {
	...
	methods: {
		...
		async submitShippingStep() {
			...
			if (!this.isMultiShipping && this.billingSame) {
