	...
	methods: {
		async createConsignments(address) {
			...
			if (this.multiShipOrder) {
				this.consignment.shippingAddress = address
				this.consignment.lineItems = [
					{
						itemId: this.consignment_data.cartItems[0].productId,
						quantity: 1
					}
				]
				const state = await this.service.createConsignments([this.consignments])
				console.log(state.data.getConsignments())
			} else {
				await this.service.updateShippingAddress(this.selectedAddress)
				await this.service.updateBillingAddress(this.selectedAddress)
			}
		},
